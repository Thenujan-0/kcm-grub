...
void KCMGRUB2::load()
{
    ...
    if (!m_osEntries.isEmpty()) {
        ...
        for (int i = 0; i < m_osEntries.size(); i++) {
            const Entry &entry = m_osEntries.at(i);
            ...
            if (i + 1 < m_osEntries.size()) {
                int n = m_osEntries.at(i + 1).level() - entry.level();
                if (n == 1) {
        ...
        int entryIndex = -1;
        for (int i = 0; i < m_osEntries.size(); i++) {
            if ((numericDefault && m_osEntries.at(i).fullNumTitle() == grubDefault) || (!numericDefault && m_osEntries.at(i).fullTitle() == grubDefault)) {
                entryIndex = i;
                break;
            }
        }
        if (entryIndex != -1) {
            const Entry &entry = m_osEntries.at(entryIndex);
            if (entry.level() == 0) {
    ...
    ui->pushbutton_remove->setEnabled(!m_osEntries.isEmpty());
...
void KCMGRUB2::save()
{
    ...
    saveAction.addArgument(QStringLiteral("rawDefaultEntry"), !m_osEntries.isEmpty() ? grubDefault.toUtf8() : m_settings.value(QStringLiteral("GRUB_DEFAULT")).toUtf8());
...
{
#if HAVE_QAPT || HAVE_QPACKAGEKIT
    QPointer<RemoveDialog> removeDlg = new RemoveDialog(m_osEntries, this);
    if (removeDlg->exec()) {
        load();
    }
    delete removeDlg;
